...
export const Address = ({ address, selectedAddressId, onAddressChange, onEdit }) => {

  

  return (
    <div className={classes.box__card}>
      <label htmlFor={`address-${address.id}`} className={classes.box__content__filter__option}>
        <input
      ...
      <div className={classes.box__card__info}>
        ...
        <p className={classes.box__card__address}>Phone Number: {address.mobile_no}</p>
      </div>
      <div className={classes.box__card__confirm}>
        {/* <button className={classes.box__card__confirm__btn}>
          Delivery to this address
        </button> */}
        <button className={classes.box__card__navigate} onClick={onEdit}>Edit Address</button>
      </div>
    </div>
  );
};
